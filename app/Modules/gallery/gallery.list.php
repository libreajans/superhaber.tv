<?php
	...
	if($_REQUEST['filter'] == 1)
	{
		...
		$_SESSION[SES]['gallery_list']['user']		= $user;
		$_SESSION[SES]['gallery_list']['status']	= $status;
		$_SESSION[SES]['gallery_list']['limit']		= $limit;
		$_SESSION[SES]['gallery_list']['keyword']	= $keyword;
		$_SESSION[SES]['gallery_list']['filter']	= 1;
	}

	foreach($array_users as $k => $v)
	{
		if($_SESSION[SES]['gallery_list']['filter'] == 1) { $selected = ''; if($_SESSION[SES]['gallery_list']['user'] == $k) $selected = 'selected'; }
		$list_users.= '<option '.$selected.' value="'.$k.'">'.$v.'</option>'. "\n";
	}

	foreach($array_content_status as $k => $v)
	{
		if($_SESSION[SES]['gallery_list']['filter'] == 1) { $selected = ''; if($_SESSION[SES]['gallery_list']['status'] == $k) $selected = 'selected'; }
		$list_content_status.= '<option '.$selected.' value="'.$k.'">'.$v.'</option>'. "\n";
	}

	foreach($array_limits as $k => $v)
	{
		if($_SESSION[SES]['gallery_list']['filter'] == 1) { $selected = ''; if($_SESSION[SES]['gallery_list']['limit'] == $k) $selected = 'selected'; }
		$list_content_limits.= '<option '.$selected.' value="'.$k.'">'.$v.'</option>'. "\n";
	}

	//filtreleme istenmişse, filtrelenmiş sonuçları gösteriyoruz
	if($_SESSION[SES]['gallery_list']['filter'] == 1)
	{
		$list = $_content->gallery_list_small
		(
			$_SESSION[SES]['gallery_list']['user'],
			$_SESSION[SES]['gallery_list']['status'],
			$_SESSION[SES]['gallery_list']['keyword'],
			$_SESSION[SES]['gallery_list']['limit']
		);
	}
	else
	{
		...
		$list = $_content->gallery_list_small('-1', '-1', '', 100);
	}
	...
	if($list <> false)
	{
		for($i = 0; $i < $adet; $i++)
		{
			...
			$link_edit_url				= LINK_ACP.'&amp;view=gallery&amp;do=edit&amp;id='.$content_id;
			$link_delete_url			= LINK_ACP.'&amp;view=gallery&amp;action=delete&amp;id='.$content_id;
			$confirm_sorusu				= 'Galeri kaydını silmek istediğinizden emin misiniz?';
			...
			if($_auth['gallery_delete'] <> '1') $hata_delete = '1';
			if($_auth['gallery_edit'] <> '1') 	$hata_edit = '1';
	...
	<div class="box box-primary">
		...
		<form action="<?=LINK_ACP?>" name="form1" method="get">
			...
			<div class="box-header">
				<h3 class="box-title">Galeri Yönetimi - Hızlı Filtrele</h3>
			</div>
					...
					<table id="recordList_Content" class="table table-bordered table-striped">
						<thead>
							<tr>
								...
								<th width="80"><i class="ion ion-person"></i> Ekleyen</th>
								<th width="40" class="right"><i class="ion ion-refresh"></i></th>
